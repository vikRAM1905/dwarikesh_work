...
import 'package:dwarikesh/model/activity_or_village_cdo_report_model.dart';
...
class ActivityorVillageCDOReportController extends GetxController {
  ...
  void apiGetList() async {
    ...
    Request request = Request(url: urlActivityorVillagewiseCDOReport, body: {
      "villageid": villageID,
    ...
    request.post().then((response) {
      ...
      if (response.statusCode == 200) {
        if (response.body.isNotEmpty) {
          ActivityorVillageCdoReportModel listModel =
              ActivityorVillageCdoReportModel.fromJson(
                  json.decode(response.body));
          if (listModel.status == true) {
            resListData.value = listModel.data!;

            for (int i = 0; i < resListData.length; i++) {
              print('DATA VALUE ${resListData[i].totalactivity}');
            }
          } else
